...
async function reset() {
  await db.query('TRUNCATE ticket_scans, email_send_log, webhook_logs, tickets, events RESTART IDENTITY CASCADE');
  await db.query('DELETE FROM settings');
  await db.query("INSERT INTO settings (org_name, auto_send_emails) VALUES ('Test Org', false)");
  await db.query(`INSERT INTO events (name, event_date, sku, active, archived) VALUES
    ('Two Day Pass',  '2026-09-01', 'CDS-2DAY', true,  false),
    ('Saturday Pass', '2026-09-01', 'CDS-SAT',  true,  false),
    ('Archived Show', '2025-09-01', 'CDS-OLD',  true,  true),
    ('Inactive Show', '2026-10-01', 'CDS-OFF',  false, false)`);
}
...
(async () => {
  ...
  // ---- B: duplicate ----
  console.log('\nB. duplicate replay');
  r = await orders.processOrderCreate(order(1001, [{ id: 5001, sku: 'CDS-2DAY', quantity: 2 }]), { source: 'retry' });
  ...
  r = await orders.processOrderCreate(order(1002, [{ id: 5003, sku: 'CDS-OLD', quantity: 2 }]), { source: 'retry' });
  check('archived event -> no tickets', r.outcome === 'no_ticket_items', r.outcome);
  r = await orders.processOrderCreate(order(1003, [{ id: 5004, sku: 'CDS-OFF', quantity: 1 }]), { source: 'retry' });
  ...
  r = await orders.processOrderStatusChange({
    ...
    status: 'refunded', webhookType: 'refund', source: 'live',
  });
  ...
  await reset();
  r = await orders.processOrderCreate(order(2001, [{ id: 6001, sku: 'CDS-2DAY', quantity: 2 }]), { source: 'live' });
  const [t1, t2] = r.tickets;
  await db.query('INSERT INTO ticket_scans (ticket_id, scan_date) VALUES ($1, NOW())', [t1.id]);
  r = await orders.processOrderStatusChange({
  ...
  await reset();
  await db.query(`INSERT INTO tickets (event_id, name, email, uuid, shopify_order_id) VALUES
    (1,'Legacy A','a@x.com','uuid-a','3001'), (1,'Legacy B','b@x.com','uuid-b','3001')`);
  r = await orders.processOrderStatusChange({
  ...
  const results = await Promise.all([
    orders.processOrderCreate(payload, { source: 'live' }),
    orders.processOrderCreate(payload, { source: 'live' }),
    orders.processOrderCreate(payload, { source: 'live' }),
  ]);
  ...
  await reset();
  await db.query('UPDATE settings SET auto_send_emails = true');
  r = await orders.processOrderCreate(order(5001, [{ id: 9001, sku: 'CDS-2DAY', quantity: 4 }]), { source: 'live' });
  ...
  check('one log row per message, marked failed',
    elog.length === 1 && elog[0].success === false && +elog[0].c === 1, JSON.stringify(elog));

  // ---- I: validation ----
  console.log('\nI. validation');
  await reset();
  r = await orders.processOrderCreate({ id: 6001, customer: { first_name: 'X' } }, { source: 'live' });
  check('missing line_items rejected', r.outcome === 'invalid', r.outcome);
  r = await orders.processOrderCreate({ id: 6002, line_items: [], customer: {} }, { source: 'live' });
